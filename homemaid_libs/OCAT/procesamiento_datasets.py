...
def extraer_muestra(corpus:pd.DataFrame, target:pd.DataFrame, proporcion:list, tamano_conjunto:int, tamano_muestra:float=0.5):
    ...
    print("column size:", len(columns))

    muestra_corpus_dataframe = pd.DataFrame(muestra_corpus, columns=columns)
    muestra_target_dataframe = pd.DataFrame(muestra_target[:], columns=["Class"])

    validacion_corpus_dataframe = pd.DataFrame(match_datos, columns=columns)
    validacion_target_dataframe = pd.DataFrame(match_targets[:], columns=["Class"])

    train_set = pd.concat([muestra_corpus_dataframe, muestra_target_dataframe], axis=1)
    test_set = pd.concat([validacion_corpus_dataframe, validacion_target_dataframe], axis=1)
